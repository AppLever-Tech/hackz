...
const CONTROL_PLANE_APP = 'hackz-analysis-control-plane';
...
export function controlPlaneProjectId(): string {
  ...
  if (id.length === 0) {
    throw new AnalysisError('CONTROL_PLANE_UNAVAILABLE', 'HACKZ_CONTROL_PLANE_PROJECT_ID is required.');
  }
  return id;
}
...
export function tenantApp(tenantRegistryId: string, projectId: string): App {
  return appNamed(`hackz-analysis-tenant-${tenantRegistryId.trim()}`, projectId.trim());
}
