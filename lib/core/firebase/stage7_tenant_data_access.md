...
| **Remaining org data** | Attachments, domains, org settings, departments, requests, coordinators | `HackzFirebase.current` | Org settings cache cleared on rebind. Tenant settings live in `hkzOrgSettings/org_settings`. Onboarding mirrors catalog `hkzOrganizations` into the tenant project and seeds `hkzOrgSettings` there so Manage College works after Open organisation. |
