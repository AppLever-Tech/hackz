...
class TeamIdeaPreviewRoute extends StatelessWidget {
  const TeamIdeaPreviewRoute({
    super.key,
    required this.preview,
    this.onOpenCreator,
  });

  final TeamIdeaPreview preview;
    ...
    return ListView(
      ...
      children: <Widget>[
        ...
        Wrap(
          ...
          runSpacing: 8,
          children: <Widget>[
            _pill(AppIcons.statusUnderReview, StatusStyles.labelForIdeaStatus(idea.status)),
            _pill(
              AppIcons.scoring,
              preview.avgScore == null ? 'No score' : 'Avg ${preview.avgScore!.toStringAsFixed(1)}',
            ),
            if (preview.paymentStatus != null)
              _pill(AppIcons.payments, _paymentLabel(preview.paymentStatus!)),
          ],
        ),
  ...
  static String _paymentLabel(PaymentRecordStatus status) {
